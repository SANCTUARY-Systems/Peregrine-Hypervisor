 ...
 * Returns the VM's own ID.
 */
static inline ffa_vm_id_t pg_vm_get_id(void)
{
 ...
 */
static inline struct ffa_value ffa_rxtx_map(pg_ipaddr_t send, pg_ipaddr_t recv)
{
	return ffa_call(
		(struct ffa_value){.func = FFA_RXTX_MAP_64,
				   .arg1 = send,
				   .arg2 = recv,
				   .arg3 = PG_MAILBOX_SIZE / FFA_PAGE_SIZE});
}
 ...
 * became writable.
 */
static inline int64_t pg_mailbox_writable_get(void)
{
	return pg_call(PG_MAILBOX_WRITABLE_GET, 0, 0, 0);
}
 ...
 * waiter otherwise.
 */
static inline int64_t pg_mailbox_waiter_get(ffa_vm_id_t vm_id)
{
	return pg_call(PG_MAILBOX_WAITER_GET, vm_id, 0, 0);
}
 ...
 */
static inline int64_t pg_interrupt_enable(uint32_t intid, bool enable,
					  enum interrupt_type type)
{
	return pg_call(PG_INTERRUPT_ENABLE, intid, enable, type);
}

/**
 * Gets the ID of the pending interrupt (if any) and acknowledge it.
 *
 * Returns PG_INVALID_INTID if there are no pending interrupts.
 */
static inline uint32_t pg_interrupt_get(void)
{
	return pg_call(PG_INTERRUPT_GET, 0, 0, 0);
}
 ...
 */
static inline int64_t pg_interrupt_inject(ffa_vm_id_t target_vm_id,
					  ffa_vcpu_index_t target_vcpu_idx,
					  uint32_t intid)
{
	return pg_call(PG_INTERRUPT_INJECT, target_vm_id, target_vcpu_idx,
		       intid);
}
 ...
 */
static inline int64_t pg_debug_log(char c)
{
	return pg_call(PG_DEBUG_LOG, c, 0, 0);
}
