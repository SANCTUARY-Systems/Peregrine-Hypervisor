#include "pg/arch/virt_devs/iomux.h"    /* virt pin controller API  */
...
static uint8_t dev_regs[V_IOMUX_DEVS][V_IOMUX_SIZE];

/* register state spinlock (per virt device) */
static struct spinlock spinlocks[MAX(V_IOMUX_DEVS, 1)];
...
/* virt_iomux_init - initializes virtual pin controller device
 ...
 */
int64_t
virt_iomux_init(struct virt_dev *dev,
                size_t          slots_left)
{
    dlog_debug("    + iomux\n");

    /* sanity check */
    if (V_IOMUX_DEVS > slots_left || !slots_left) {
        dlog_error("Insufficient virt device descriptor slots remaining\n");
        return -1;
    }

    /* register each device instance */
    for (size_t i = 0; i < V_IOMUX_DEVS; i++) {
        /* initialize internal register state */
        memset_peregrine(dev_regs[i], 0x00, V_IOMUX_SIZE);

        /* initialize spinlock */
        spinlocks[i] = SPINLOCK_INIT;

        /* initialize virtual device descriptor */
        dev[i].name       = "virt-iomux";
        dev[i].minor      = i;
        dev[i].addr_start = V_IOMUX_BASE + i * V_IOMUX_SIZE;
        dev[i].addr_end   = V_IOMUX_BASE + (i + 1) * V_IOMUX_SIZE;
        dev[i].access     = access;
    }
    ...
    /* initialize virtual device descriptor */
#ifdef V_IOMUX_DIRMAP
    dev->name       = "virt-iomux[DM]";
#else
    dev->name       = "virt-iomux[PT]";
#endif /* V_IOMUX_DIRMAP */
    dev->minor      = 0;
    dev->addr_start = V_IOMUX_BASE;
    dev->addr_end   = V_IOMUX_BASE + V_IOMUX_SIZE;
    dev->access     = access;
