...
TEST_F(vm, vm_unmap_hypervisor_not_mapped)
{
	...
	EXPECT_TRUE(vm_init_next(1, 1, &cpus[0], &ppool, &vm));
...
TEST_F(vm, vm_boot_order)
{
	 ...
	 */
	EXPECT_TRUE(vm_init_next(1, 1, &cpus[0], &ppool, &vm_cur));
	vm_cur->boot_order = 1;
	...
	EXPECT_TRUE(vm_init_next(1, 1, &cpus[1], &ppool, &vm_cur));
	vm_cur->boot_order = 3;
	vm_update_boot(vm_cur);
	expected_final_order.push_back(vm_cur);
	...
	for (int i = 0; i < 2; i++) {
		EXPECT_TRUE(vm_init_next(1, 1, &cpus[i + 2], &ppool, &vm_cur));
		vm_cur->boot_order = 2;
		vm_update_boot(vm_cur);
		expected_final_order.push_back(vm_cur);
	}
